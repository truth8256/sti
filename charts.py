...
def render_age_highlight_chart(pop_df: pd.DataFrame, *, box_height_px: int = 320, width_px: int = 260):
    ...
    ratios_all  = [(v / total_v * 100.0) if total_v > 0 else 0.0 for v in vals_all]
    ...
    inner_r = max(50, min(outer_r - 40, 90))
    mark_common = dict(innerRadius=inner_r, outerRadius=outer_r, padAngle=0.003, cornerRadius=0, strokeJoin="round")
    ...
    df_main = pd.DataFrame({"연령": labels_main, "명": vals_main, "비율": [round(r, 6) for r in ratios_all[:3]], "순서": [0,1,2]})
    df_extra = pd.DataFrame({"연령": ["60~64세"], "명": [a6064], "비율": [round(ratios_all[3], 6)], "순서": [3]})

    df_main_op = df_main.copy()
    df_main_op["투명도"] = 1.0 if focus == "전체" else df_main_op["연령"].eq(focus).map({True: 1.0, False: 0.35})
    ...
    arcs_extra = (
        base_extra.mark_arc(**mark_common, stroke="white", strokeWidth=1)
        .encode(
    ...
    arcs_main = (
        base_main.mark_arc(**mark_common, stroke="white", strokeWidth=1)
        .encode(
            theta=alt.Theta("비율:Q"),
    ...
    highlight = (
        base_high.mark_arc(**mark_common, stroke="#111827", strokeWidth=2.5, fillOpacity=0.0001)
        .encode(theta=alt.Theta("비율:Q"), color=alt.Color("연령:N", scale=alt.Scale(domain=color_domain, range=color_range), legend=None),
                order=alt.Order("순서:Q", sort="ascending"))
    )

    if focus == "전체":
        big, small = f"{int(round(total_v)):,}", ""
    else:
        i = labels_main.index(focus)
        big, small = f"{ratios_all[i]:.1f}%", f"{int(round(vals_main[i])):,}명"

    center_big = alt.Chart(pd.DataFrame({"x":[0]})).mark_text(fontSize=20, fontWeight="bold", align="center", baseline="middle", color="#0f172a") \
        .encode(x=alt.value(cx), y=alt.value(cy - 6), text=alt.value(big))
    center_small = alt.Chart(pd.DataFrame({"x":[0]})).mark_text(fontSize=12, align="center", baseline="middle", color="#475569") \
        .encode(x=alt.value(cx), y=alt.value(cy + 14), text=alt.value(small))

    st.altair_chart(arcs_extra + arcs_main + highlight + center_big + center_small, use_container_width=True)
